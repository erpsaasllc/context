        ...
        <div class="space-y-3">
            ...
            <x-filament::form wire:submit.prevent="createApiToken">
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-context-input id="name" type="text" class="mt-1 block w-full"
                        ...
                        <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach (ERPSAAS\Context\Context::$permissions as $permission)
                                <label class="flex items-center">
                                    <x-context-checkbox wire:model.defer="createApiTokenForm.permissions"
                                        :value="$permission" />
            ...
            <!-- Token Value Modal -->
            <x-context-dialog-modal wire:model="displayingToken" maxWidth="md"
            ...
            <x-context-dialog-modal wire:model="managingApiTokenPermissions" maxWidth="md"
                class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
                <x-slot name="title">
                ...
                <x-slot name="content">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (ERPSAAS\Context\Context::$permissions as $permission)
                            <label class="flex items-center">
                                <x-context-checkbox wire:model.defer="updateApiTokenForm.permissions"
                                    :value="$permission" />
                                <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $permission }}</span>
                            </label>
                        @endforeach
            ...
            <x-context-confirmation-modal wire:model="confirmingApiTokenDeletion" maxWidth="md"
                class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
                <x-slot name="title">
                    {{ __('Delete API Token') }}
                </x-slot>
