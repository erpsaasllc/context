...
use ERPSAAS\Context\Actions\UpdateCompanyEmployeeRole;
use ERPSAAS\Context\Contracts\AddsCompanyEmployees;
use ERPSAAS\Context\Contracts\InvitesCompanyEmployees;
use ERPSAAS\Context\Contracts\RemovesCompanyEmployees;
use ERPSAAS\Context\Features;
use ERPSAAS\Context\Context;
use ERPSAAS\Context\Role;
...
class CompanyEmployeeManager extends Component
{
    ...
    public function cancelCompanyInvitation($invitationId)
    {
        if (! empty($invitationId)) {
            $model = Context::companyInvitationModel();
    ...
    public function manageRole($userId)
    {
        ...
        $this->managingRoleFor = Context::findUserByIdOrFail($userId);
     ...
     *
     * @param  \ERPSAAS\Context\Actions\UpdateCompanyEmployeeRole  $updater
     * @return void
     */
     ...
     *
     * @param  \ERPSAAS\Context\Contracts\RemovesCompanyEmployees  $remover
     * @return void
     */
    ...
    public function removeCompanyEmployee(RemovesCompanyEmployees $remover)
    {
        $remover->remove(
            $this->user,
            $this->company,
            $user = Context::findUserByIdOrFail($this->companyEmployeeIdBeingRemoved)
        );
    ...
    {
        return collect(Context::$roles)->transform(function ($role) {
